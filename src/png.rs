...
impl Png {
    ...
    pub fn append_chunk(&mut self, chunk: Chunk) {
        // todo correct this chunk is valid
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self.chunks.iter().position(|x| x.chunk_type().bytes() == chunk_type.as_bytes()).unwrap();
        let res = self.chunks.remove(index);
        Ok(res)
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks.iter().find(|&e| e.chunk_type().bytes() == chunk_type.as_bytes())
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunk_bytes: Vec<u8> = self.chunks
            ...
            .collect();
        let res = self.header.iter()
            .cloned().chain(chunk_bytes).collect();
        res
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        let mut res = Vec::new();
        while i < bytes.len() {
            let mut buffer_len: [u8; 4] = [0, 0, 0, 0];
            reader.read_exact(&mut buffer_len)?;

            let data_length = u32::from_be_bytes(buffer_len) as usize;
            let index: usize = i + data_length + 12;
            if index > bytes.len() {
                break;
            }
            let x = &bytes[i..index];
            let mut result = Chunk::try_from(x);
            if let Ok(re) = result {
                res.push(re);
            } else {
                return Err("chunk decode error".into());
            }

            reader.consume(index - i - 4);
            i = index;
        }
        Ok(Png {
            header: buffer,
